    ...
    class TestScene extends Phaser.Scene {
        ...
        tapUpdate(time, button) {
            ...
            if (this.stateRunning && !this.stateShowResults) {
                ...
                this.mouseButton.push(this.add.text(x - 2, 360, `+${frame}`, this.smallText));
            }
            if (!this.stateRunning) {
                this.results = null;
                ...
                if (c.firstFrame > jf.earlyFrame - 1) {
                    if (c.firstFrame < jf.latestFrame + 1) {
                        ...
                        if (c.firstFrame > jf.justFrame && jf.latestFrame > jf.justFrame) { //I presume push should always be zero and this is called once...but... can change later if need be.
                            ...
                            if (this.pushCount < 1)
                                c.chancePush = this.pushCount; // going to push the next frame
                        }
                        if (c.firstFrame < jf.earlyFrame)
                            c.chanceEarly = jf.earlyFrame - c.firstFrame; // maybe early                        
                    }
                    else
                        c.chanceLate = 1; // late and clamed = dead.
                    c.claimedFrame = this.nextUnclaimed++; // was on time or late so claimed.
                }
                else
                    c.chanceEarly = 1; // too early - not claimed... ?? maybe change to some other flag...hmmm. but then the next one might be late.
            } //else we done...
        }
    }
})(Tappy || (Tappy = {}));
